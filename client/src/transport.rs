...
impl OnionConnection {
    pub async fn new(with_host: &str) -> Self {
    ...
    /// Returns the get circuit information of this [`OnionConnection`].
    pub async fn get_circuit_information(&self) -> Vec<CircuitInfo> {
        let circuit = self.stream.circuit().path_ref();
    ...
    pub async fn make_request(self) -> Result<()> {
        let (mut request_sender, connection) =
            hyper::client::conn::http1::handshake(TokioIo::new(self.stream)).await?;
